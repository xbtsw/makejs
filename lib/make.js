...
Make.prototype.depCache = function (input) {
    var key;
    if (input === undefined) {
        var ret = {};
        for (key in this.dependencyCache) {
            ret[key] = this.dependencyCache[key].toArray();
        }
        return ret;
    }
    for (key in input) {
        if (key in this.dependencyCache) {
            this.dependencyCache[key].absorb(input[key]);
        }
        else {
            this.dependencyCache = new StringSet(input[key]);
        }
    }
};
            ...
            (function (t) {
                ...
                getStat(t, function (err, targetStat) {
                    ...
                    async.map(makeInst.targets[t].dependsOn.toArray(), getStat, function (err, stats) {
                        ...
                        else if (t in makeInst.dependencyCache && !makeInst.dependencyCache[t].isSubsetOf(makeInst.targets[t].dependsOn)) {
                            shouldRun = true;
                        }
                        ...
                        if (!isPhony(t)) {
                            fs.readFile(t, function (e) {
                                if (e !== null && e.code === 'ENOENT') {
                                    ...
                                    makeInst.dependencyCache[t] = makeInst.targets[t].dependsOn.clone();
                                    if (t === target) {
                                        callback(err);
                                    }
                                    else {
